...
以下规则由用户在实施阶段明确指定，优先于本文后续所有单项 Prompt 中的深色、3D、2.5D、等距或霓虹夜景描述：

- 所有 `CODEX_IMAGE_GEN_TASK` 均使用明亮的纯 2D 卡通插画；使用扁平形状、清晰轮廓与少量柔和二维阴影，不使用 3D 渲染或写实材质。
- 背景以白色、近白色或浅粉彩为主，尽量使用透明、磨砂玻璃式线框与轻盈留白；避免大面积深色背景。
- 每张图必须准确表达对应玩法，不出现标题文字、第三方 Logo、水印、真实球队/品牌或受保护角色。
- 所有单项 Prompt 只保留其玩法主体和构图要求；与本覆盖规则冲突的风格、光照、背景和材质描述一律忽略。
- 源图由 ImageGen 生成并保留；仓库脚本只负责从源图裁切和压缩 `cover.webp` / `og.webp`，不得重绘或覆盖 `source.png`。
...
Garden Logic 使用与全站一致的明亮 2D 卡通基线，并加入“数字花园”辨识度：

- 白色、近白色和浅薄荷背景，辅以透明线框与充足留白。
- 植物、星图、果实、符号和几何线条只作为原创抽象主题。
- 重点色使用薄荷绿、杏橙、柔紫与金黄。
- 游戏棋盘保持高对比、低噪音；装饰不能干扰数字或符号识别。
- 卡片封面统一采用纯 2D 扁平卡通插画，不在图片中绘制标题文字。
